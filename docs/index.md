...
| `Object[]` | 对象数组 | 逗号分隔 `key:value` 槽位，每槽位一个对象，n 个逗号 → n+1 个元素，空槽位为 `{}` |
...
- **Object[]**: 单元格为逗号分隔的 `key:value` 槽位，每个槽位是一个对象；n 个逗号 → n+1 个元素（含末尾逗号），空槽位为 `{}`，空单元格为 `[]`。如 `num:1,,str:a,` 生成 4 个元素 `[{num:1}, {}, {str:'a'}, {}]`；跨所有行合并 key 生成元素类型，字段类型为 `类型名[]`
- **数组空数据约定**: 所有数组类型（`String[]`/`Number[]`/`Enum[]`/`RefEnum[...] []`/`Object[]`）——原始值统一清洗（去除 `\r\n` 等换行符、首尾空格）；空单元格生成 `[]`；非空时 n 个逗号 → n+1 个槽位，所有槽位（含连续、首尾多余逗号产生的）全部保留，保证并行数组按下标对齐。空槽位按类型自身零值生成：String/Enum/RefEnum 为 `''`，Number 为 `0`，Object[] 为 `{}`，**不生成 `null`**
