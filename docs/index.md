...
- **Object[]**: 对象之间用**分号 `;` 分隔**（逗号留给对象内的 `key:value`，与 `Object` 类型规则相同，多余逗号产生的空键值段忽略）。如 `num:1,,str:a;num:2` 生成 `[{num:1, str:'a'}, {num:2}]`；对象级空槽位（`a:1;;b:2`）按 n+1 规则保留为空对象 `{}`，空单元格为 `[]`；跨所有行合并 key 生成元素类型，字段类型为 `类型名[]`
