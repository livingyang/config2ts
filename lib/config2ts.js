        ...
        if (i === 0) {
            ...
            for (const k in d) {
                ...
                if (convert[k] === ObjectStr) {
                    ...
                    if (template.isArray) {
                        const entries = d[k] === "" ? [] : d[k].split(TemplateEntrySeparator).map((part) => parseTemplateEntry(part));
                        ...
                        const entry = parseTemplateEntry(d[k]);
                        if (d[k] !== "" && entry.baseId === "") {
...
// The part before the first '|' is the base row id; the part after it reuses
// the Object rule for the override pairs. No '|' means zero overrides, which
// is equivalent to a plain Ref.
function parseTemplateEntry(str) {
    ...
    const overrides = parseObject(str.slice(separatorIndex + 1));
