...
function scanDir(dir, basePath, refPrefix, groupMap) {
    const node = {};
...
function serializeAssetNode(node, indent) {
    if (node === null)
        return `${indent}{}`;
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if ("path" in value && "type" in value) {
            lines.push(`${indent}${serializedKey}: {path:${json5.stringify(value.path)},type:${json5.stringify(value.type)}} as AssetMeta${comma}`);
        }
        else {
            lines.push(`${indent}${serializedKey}: {`);
            lines.push(serializeAssetNode(value, indent + "    "));
...
function assets2ts(assetsDir) {
    ...
    const allTypes = new Set([...Object.values(EXT_MAP), "other"]);
