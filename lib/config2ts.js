...
function isStructuralType(typeStr) {
...
function splitBracketAware(str, separators) {
    const parts = [];
    let depth = 0;
    let start = 0;
...
function collectCsvReferences(filePath) {
    ...
    const references = [];
    (0, d3_dsv_1.csvParse)(fileString, function (d, i) {
        if (i === 0) {
            for (const k in d) {
                const typeStr = d[k];
                const ref = parseRef(typeStr);
...
function sortFilesByReferences(fileList) {
    ...
    function visit(base, trail) {
        var _a, _b;
        const current = (_a = state.get(base)) !== null && _a !== void 0 ? _a : 0;
        if (current === 2) {
            return;
        }
        if (current === 1) {
        ...
        state.set(base, 1);
        trail.push(base);
        for (const dep of (_b = depsByBase.get(base)) !== null && _b !== void 0 ? _b : []) {
            if (fileByBase.has(dep)) {
                visit(dep, trail);
            }
        }
